...
def cv_with_random_model(df, target_column, feature_nums, class_nums, threshold_nums, models_nums, k):
...
if __name__ == "__main__":
    ...
    y = df[target_column]
    feature_nums = len(X.columns)
    class_nums = len(set(y))

    # 計算門檻時迭代 20 次,每次跑 1000 個模型，最後在生成 125 (一個 fold 25 個)個基本模型
    fold_accuracies, fold_nb_models_prior, fold_nb_models_likelihood = cv_with_random_model(df, target_column, feature_nums, class_nums, 20, 1000, k)
    ...
    plt.ylabel("Density")
    plt.grid(True)
    plt.savefig(os.path.join(parent_path, "charts", "TRENB_prior_c1.jpg"))
    plt.close()